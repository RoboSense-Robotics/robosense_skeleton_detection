//
// ROS1 版本的 RvizDisplay - 发布骨架 Marker 与 2D 图像到 RViz
// 对应 ROS2 的 RvizDisplay
//

#ifndef POSE_DETECTION_ROS_RVIZ_DISPLAY_H
#define POSE_DETECTION_ROS_RVIZ_DISPLAY_H
...
class RosRvizDisplay {
public:
  using Ptr = std::shared_ptr<RosRvizDisplay>;

  RosRvizDisplay(const ros::NodeHandle& nh) : nh_(nh) {
...
private:
  ros::Publisher left_pd_pub_;
  ros::Publisher right_pd_pub_;
  ros::Publisher fusion_3d_marker_pub_;
  ...
  std::vector<std::pair<int, int>> coco_133_joint_links_ = {
      ...
      {2, 4},   {3, 5},   {4, 6},   {15, 17}, {15, 18}, {15, 19}, {16, 20}, {16, 21},
      {16, 22}, {91, 92}, {92, 93}, {93, 94}, {94, 95}, {91, 96}, {96, 97}, {97, 98},
      {98, 99}, {91, 100}, {100, 101}, {101, 102}, {102, 103}, {91, 104}, {104, 105},
      {105, 106}, {106, 107}, {91, 108}, {108, 109}, {109, 110}, {110, 111}, {112, 113},
      {113, 114}, {114, 115}, {115, 116}, {112, 117}, {117, 118}, {118, 119}, {119, 120},
      {112, 121}, {121, 122}, {122, 123}, {123, 124}, {112, 125}, {125, 126}, {126, 127},
      {127, 128}, {112, 129}, {129, 130}, {130, 131}, {131, 132}};

  std::vector<std::pair<int, int>> halpe_26_joint_links_ = {
      {19, 11}, {19, 12}, {19, 18}, {4, 6},  {2, 4},  {2, 0},  {0, 1},  {1, 3},
      {3, 5},   {0, 18},  {0, 17},  {1, 2},  {5, 7},  {7, 9},  {6, 8},  {8, 10},
      {6, 18},  {5, 18},  {11, 13}, {13, 15}, {12, 14}, {14, 16}, {15, 24}, {16, 25},
      {15, 20}, {15, 22}, {16, 23}, {16, 21}};

  std::vector<std::pair<int, int>> line_set_ = {
      {0, 1}, {1, 2}, {2, 3}, {1, 4}, {0, 4}, {4, 5}, {5, 6}, {0, 9},
      {9, 10}, {10, 11}, {0, 12}, {12, 13}, {13, 14}};

  std::vector<int> display_points_idx_ = {0, 1, 2, 3, 4, 5, 6, 9, 10, 11, 12, 13, 14};

  static std_msgs::ColorRGBA createColor(float r, float g, float b, float a = 1.0f) {
  ...
  std::map<rally::CameraEnum, std_msgs::ColorRGBA> ac_name_color_map_;
...
#endif  // POSE_DETECTION_ROS_RVIZ_DISPLAY_H
